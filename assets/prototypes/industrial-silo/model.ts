...
function cone(root: Group, m: CargoMaterials, top: number): void {
  ...
  for (let index = 0; index < sides; index += 1) {
    ...
    const midR = (RADIUS + apexR) * 0.5
    const slant = Math.hypot(CONE, RADIUS - apexR)
    const panel: Vec2[] = [
      [-(Math.PI * 2 * RADIUS) / sides * 0.52, slant * 0.5],
      [(Math.PI * 2 * RADIUS) / sides * 0.52, slant * 0.5],
      [(Math.PI * 2 * apexR) / sides * 0.6, -slant * 0.5],
      [-(Math.PI * 2 * apexR) / sides * 0.6, -slant * 0.5],
    ]
    ...
    mesh.rotateX(Math.atan2(RADIUS - apexR, CONE))
    root.add(mesh)
  }
}
...
function build(): {
  ...
  for (let index = 0; index < 4; index += 1) {
    ...
    for (const [lift, tilt] of [[LEG * 0.36, 0.34], [LEG * 0.7, -0.34]] as const) {
  ...
  body.add(cylinder(m.shell, RADIUS, BARREL, [0, shoulder + BARREL * 0.5, 0], AXIS_Y, 22))
  body.add(cylinder(m.graphiteEdge, RADIUS + 0.03, 0.1, [0, shoulder + 0.02, 0], AXIS_Y, 22))
  for (const fraction of [0.32, 0.66]) {
    body.add(cylinder(m.shellShade, RADIUS + 0.012, 0.05, [0, shoulder + BARREL * fraction, 0], AXIS_Y, 22))
  }
  // Roof: a shallow cap, a fill head, and a dust filter drum.
  body.add(cylinder(m.shellLight, RADIUS - 0.08, 0.16, [0, shoulder + BARREL + 0.06, 0], AXIS_Y, 22))
  ...
  for (let index = 0; index < 4; index += 1) {
    box(body, m.graphiteEdge, [0.13, 0.07, 0.13], [lineX, 0.6 + index * 1.1, lineZ], {
      chamfer: 0.024, fillet: 0.009, bevel: 0.008,
    })
  }

  // Discharge head under the cone: gate frame, chute, and controls.
  const apex = LEG
  ...
  box(body, m.shellShade, [0.42, 0.28, 0.34], [0, apex - 0.24, 0], {
  ...
  statusLens(body, m, [0.08, 0.04], [0.44, apex + 0.22, 0.075], m.amber, 'front')
  member(body, m.steel, [0.34, apex - 0.03, 0.1], [0.1, apex - 0.03, 0.1], 0.04, 0.05)

  // Slide gate: its own assembly, so it can be pulled open.
  gate.position.set(0, apex - 0.03, 0)
  ...
  radialPlaque(body, m, label, [0.5, 0.24], RADIUS, shoulder + BARREL * 0.58, 0.62, m.ink)
  paintMark(body, m.amberPaint, slashProfile(0.14, 0.5, 0.42), [-0.2, shoulder + BARREL * 0.3, RADIUS + 0.004], 'front', 0.013)
  paintMark(body, m.amberPaint, slashProfile(0.07, 0.5, 0.42), [0.02, shoulder + BARREL * 0.3, RADIUS + 0.004], 'front', 0.013)
