...
function plinth(hull: Group, m: CargoMaterials, bundle: CargoMaterialBundle): void {
  ...
  for (const side of [-1, 1] as Side[]) {
    ...
    for (const x of [-0.5, 0.5]) {
      forkPocket(hull, m, [0.5, 0.19], 0.42, [x, SKIRT * 0.52, side * (DEPTH * 0.5 - 0.02)], face)
    }
    const stripe = addStripeDecal(bundle, { count: 5, lean: side })
    plaque(hull, m, stripe, [0.42, 0.11], [side * -0.92, SKIRT * 0.52, side * (DEPTH * 0.5 - 0.02)], face, m.ink)
  }
  // Corner feet in painted caution, the part a fork actually scrapes.
  for (const sx of [-1, 1] as Side[]) {
    for (const sz of [-1, 1] as Side[]) {
      box(hull, m.amberPaint, [0.19, 0.16, 0.19], [sx * (WIDTH * 0.5 - 0.1), 0.08, sz * (DEPTH * 0.5 - 0.1)], {
        chamfer: 0.05, fillet: 0.014, bevel: 0.012,
      })
      box(hull, m.rubber, [0.17, 0.035, 0.17], [sx * (WIDTH * 0.5 - 0.1), 0.017, sz * (DEPTH * 0.5 - 0.1)], {
...
function hullBody(hull: Group, m: CargoMaterials, bundle: CargoMaterialBundle): void {
  ...
  box(hull, m.shell, [WIDTH - 0.06, bodyHeight, DEPTH - 0.06], [0, bodyY, 0], {
    chamfer: 0.11,
    fillet: 0.035,
    bevel: 0.024,
    capChamfer: 0.08,
  ...
  for (const sx of [-1, 1] as Side[]) {
    for (const sz of [-1, 1] as Side[]) {
      box(hull, m.graphiteEdge, [0.2, bodyHeight - 0.02, 0.2], [
        sx * (WIDTH * 0.5 - 0.12), bodyY, sz * (DEPTH * 0.5 - 0.12),
  ...
  louvreVent(hull, m, [0.5, 0.52], [-0.46, bodyY, frontZ], 5, 'front')
  box(hull, m.graphite, [0.34, 0.42, 0.06], [0.34, bodyY + 0.02, frontZ], { chamfer: 0.06, fillet: 0.016, bevel: 0.012 })
  statusLens(hull, m, [0.16, 0.05], [0.34, bodyY + 0.15, frontZ + 0.03], m.cyan, 'front')
  const label = addLabelDecal(bundle, { variant: 4 })
  plaque(hull, m, label, [0.34, 0.17], [0.34, bodyY - 0.09, frontZ + 0.03], 'front', m.shellLight)
  boltRun(hull, m.steel, [0.34 - 0.19, bodyY + 0.24, frontZ + 0.032], [0.34 + 0.19, bodyY + 0.24, frontZ + 0.032], 2, 0.018, 'front')

  const backZ = -(DEPTH * 0.5 - 0.03)
  for (const x of [-0.62, 0, 0.62]) {
    seam(hull, m.shell, bodyHeight - 0.2, [x, bodyY, backZ], 'back', 'along', 0.03, 0.02)
  }
  paintMark(hull, m.amberPaint, slashProfile(0.13, 0.4, 0.45), [0.72, bodyY, backZ], 'back', 0.012)
  paintMark(hull, m.amberPaint, slashProfile(0.07, 0.4, 0.45), [0.9, bodyY, backZ], 'back', 0.012)

  // Short faces: recessed grab handles at carry height.
  for (const side of [-1, 1] as Side[]) {
    const face = side > 0 ? 'right' : 'left'
    recessedHandle(hull, m, [0.44, 0.15], [side * (WIDTH * 0.5 - 0.04), bodyY + 0.06, 0], face)
    box(hull, m.graphiteEdge, [0.05, bodyHeight - 0.12, 0.24], [side * (WIDTH * 0.5 - 0.04), bodyY, 0.4], {
...
function lidBody(lid: Group, m: CargoMaterials): void {
  ...
  box(lid, m.shellLight, [WIDTH - 0.04, LID, DEPTH - 0.04], [0, LID * 0.5, DEPTH * 0.5 - 0.05], {
    chamfer: 0.09,
    fillet: 0.028,
    bevel: 0.02,
    capChamfer: 0.06,
  ...
  for (const x of [-0.55, 0.55]) {
    seam(lid, m.shellLight, DEPTH - 0.24, [x, LID, DEPTH * 0.5 - 0.05], 'top', 'along', 0.032, 0.02)
  }
  ...
  for (const sx of [-1, 1] as Side[]) {
    for (const sz of [-1, 1] as Side[]) {
      ...
      box(lid, m.shellShade, [0.3, 0.03, 0.3], [padX, LID + 0.005, padZ], {
        chamfer: 0.07, fillet: 0.018, bevel: 0.008,
      })
      box(lid, m.ink, [0.16, 0.016, 0.16], [padX, LID + 0.016, padZ], {
        chamfer: 0.04, fillet: 0.01, bevel: 0.006,
      })
    }
  }
  // Painted lift bar across the crown, plus a pair of eye bolts.
  box(lid, m.amberPaint, [0.62, 0.055, 0.11], [0, LID + 0.03, DEPTH * 0.5 - 0.05], {
    chamfer: 0.026, fillet: 0.01, bevel: 0.009,
  })
  for (const x of [-0.31, 0.31]) {
    lid.add(cylinder(m.steel, 0.032, 0.09, [x, LID + 0.05, DEPTH * 0.5 - 0.05], AXIS_Z, 10))
  }
  // Hinge knuckles on the origin edge.
  for (const x of [-0.66, 0, 0.66]) {
    lid.add(prism(m.graphiteEdge, [0.19, 0.13, 0.11], [x, LID * 0.55, 0.03], {
      chamfer: 0.035, fillet: 0.012, bevel: 0.01,
    }))
    lid.add(cylinder(m.steel, 0.031, 0.24, [x, LID * 0.55, 0], AXIS_X, 10))
  }
}
...
function build(): { root: Group; parts: CrateParts; sockets: CrateSockets; bundle: CargoMaterialBundle } {
  ...
  lid.position.set(0, HEIGHT - LID, -(DEPTH * 0.5 - 0.05))
  lidBody(lid, m)
  ...
  for (const x of [-0.72, 0, 0.72]) {
    toggleLatch(hull, m, [x, HEIGHT - LID - 0.02, DEPTH * 0.5 - 0.02], 1.15, 'front')
  }
  ...
  const sockets: CrateSockets = {
    ...
    fx_status: socket('fx_status', [0.34, SKIRT + (HEIGHT - SKIRT - LID) * 0.5 + 0.15, DEPTH * 0.5 + 0.02]),
  }
