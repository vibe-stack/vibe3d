...
function build(): { root: Group; sockets: SealedBarrelSockets; bundle: CargoMaterialBundle } {
  ...
  drum(root, m, RADIUS, BODY, [0, 0, 0], {
    hoops: [0.28, 0.52, 0.76],
    chime: 0.028,
    body: m.shellShade,
    band: m.ironOxide,
  })
  ...
  // this scale and cost a fraction of a lathe.
  ...
  root.add(cylinder(m.ironOxide, RADIUS - 0.02, 0.022, [0, BODY + 0.002, 0], AXIS_Y, 20))
  ...
  root.add(cylinder(m.amberPaint, RADIUS + 0.006, 0.035, [0, BODY - 0.045, 0], AXIS_Y, 20))
  box(root, m.graphiteEdge, [0.09, 0.11, 0.035], [0, BODY - 0.06, RADIUS + 0.012], {
    chamfer: 0.024, fillet: 0.008, bevel: 0.007,
  })
  root.add(cylinder(m.steel, 0.014, 0.05, [0, BODY - 0.06, RADIUS + 0.036], AXIS_Z, 8))

  const label = addLabelDecal(bundle, { variant: 23 })
  radialPlaque(root, m, label, [0.24, 0.11], RADIUS, BODY * 0.58, 0.5, m.ink)
  paintMark(root, m.amberPaint, slashProfile(0.08, 0.26, 0.42), [-0.04, BODY * 0.3, RADIUS + 0.002], 'front', 0.011)
  paintMark(root, m.amberPaint, slashProfile(0.04, 0.26, 0.42), [0.06, BODY * 0.3, RADIUS + 0.002], 'front', 0.011)
  statusLens(root, m, [0.05, 0.02], [-RADIUS * 0.62, BODY * 0.82, RADIUS * 0.66], m.cyan, 'front')
  for (let index = 0; index < 3; index += 1) {
    const angle = -0.9 + index * 0.9
    bolt(root, m.steel, [Math.sin(angle) * RADIUS, 0.06, Math.cos(angle) * RADIUS], 0.015, 'front')
  }

  const sockets: SealedBarrelSockets = {
    seal_band: socket('seal_band', [0, BODY - 0.045, RADIUS + 0.06]),
    stack_top: socket('stack_top', [0, BODY + 0.11, 0]),
    fx_status: socket('fx_status', [-RADIUS * 0.62, BODY * 0.82, RADIUS * 0.66 + 0.05]),
  }
