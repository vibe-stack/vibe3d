...
function bodyBuild(body: Group, m: CargoMaterials, bundle: CargoMaterialBundle): void {
  ...
  box(body, m.shellShade, [WIDTH - 0.06, 0.02, DEPTH - 0.05], [0, foot + BODY_H, 0], {
    chamfer: 0.02, fillet: 0.008, bevel: 0.006,
  })
  // Open tray above the drawer stack, with a divider.
  box(body, m.ink, [WIDTH - 0.07, TRAY - 0.02, DEPTH - 0.06], [0, foot + BODY_H + TRAY * 0.5, 0], {
    chamfer: 0.02, fillet: 0.008, bevel: 0.006,
  })
  box(body, m.shellShade, [0.02, TRAY - 0.03, DEPTH - 0.08], [WIDTH * 0.18, foot + BODY_H + TRAY * 0.5, 0], {
    chamfer: 0.006, fillet: 0.003, bevel: 0.003,
  })
  box(body, m.graphiteEdge, [0.12, 0.03, 0.05], [-WIDTH * 0.2, foot + BODY_H + 0.03, 0.02], {
  ...
  for (let index = 0; index < DRAWERS; index += 1) {
    ...
    seam(body, m.shellLight, WIDTH - 0.1, [0, y - DRAWER * 0.46, DEPTH * 0.5 + 0.006], 'front', 'across', 0.012, 0.008)
  }

  // Carry handles, one per end.
  for (const sx of [-1, 1]) {
    for (const sz of [-1, 1]) {
      box(body, m.graphiteEdge, [0.03, 0.04, 0.04], [
        sx * (WIDTH * 0.5 + 0.012), foot + BODY_H * 0.6, sz * 0.07,
      ], { chamfer: 0.01, fillet: 0.004, bevel: 0.004 })
    }
    body.add(cylinder(m.rubber, 0.017, 0.16, [sx * (WIDTH * 0.5 + 0.03), foot + BODY_H * 0.6, 0], AXIS_X, 8))
  }

  const label = addLabelDecal(bundle, { variant: 280 })
  plaque(body, m, label, [0.18, 0.055], [-WIDTH * 0.22, foot + BODY_H + TRAY - 0.02, DEPTH * 0.5 - 0.008], 'front', m.shellLight)
  statusLens(body, m, [0.04, 0.016], [WIDTH * 0.3, foot + BODY_H + TRAY - 0.02, DEPTH * 0.5 - 0.008], m.cyan, 'front')
  paintMark(body, m.amberPaint, slashProfile(0.035, 0.05, 0.45), [WIDTH * 0.36, foot + DRAWER * 0.5, DEPTH * 0.5 + 0.008], 'front', 0.007)
  for (const sx of [-1, 1]) bolt(body, m.steel, [sx * (WIDTH * 0.5 - 0.05), foot + 0.02, -(DEPTH * 0.5 + 0.002)], 0.011, 'back')
}

function lidBuild(lid: Group, m: CargoMaterials): void {
  box(lid, m.shellLight, [WIDTH + 0.01, LID, DEPTH + 0.01], [0, LID * 0.5, DEPTH * 0.5 - 0.02], {
    chamfer: 0.04, fillet: 0.014, bevel: 0.012, capChamfer: 0.026,
  })
  box(lid, m.ink, [WIDTH - 0.14, 0.018, DEPTH - 0.1], [0, LID - 0.004, DEPTH * 0.5 - 0.02], {
    chamfer: 0.03, fillet: 0.011, bevel: 0.007,
  })
  box(lid, m.shellShade, [WIDTH - 0.2, 0.014, DEPTH - 0.14], [0, LID + 0.003, DEPTH * 0.5 - 0.02], {
    chamfer: 0.026, fillet: 0.01, bevel: 0.006,
  })
  // The underside is the face an open lid shows, so it gets a tool clip strip.
  box(lid, m.graphiteEdge, [WIDTH - 0.16, 0.012, 0.05], [0, 0.004, DEPTH * 0.5 - 0.02], {
    chamfer: 0.008, fillet: 0.004, bevel: 0.003,
  })
  for (const sx of [-1, 1]) {
    lid.add(cylinder(m.steel, 0.012, 0.09, [sx * 0.16, 0.012, DEPTH * 0.5 - 0.02], AXIS_X, 6))
    lid.add(cylinder(m.steel, 0.014, 0.1, [sx * 0.2, LID * 0.5, 0.012], AXIS_X, 8))
  }
}
...
function build(): { root: Group; body: Group; lid: Group; sockets: ToolChestSockets; bundle: CargoMaterialBundle } {
  ...
  lid.position.set(0, top, -(DEPTH * 0.5 - 0.02))
  lidBuild(lid, m)
  for (const x of [-WIDTH * 0.28, WIDTH * 0.28]) {
    toggleLatch(body, m, [x, top - 0.02, DEPTH * 0.5 + 0.004], 0.42, 'front')
  }

  const sockets: ToolChestSockets = {
    lid_hinge: socket('lid_hinge', [0, top, -(DEPTH * 0.5 - 0.02)]),
