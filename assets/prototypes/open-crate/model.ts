...
function interior(hull: Group, m: CargoMaterials): void {
  ...
  for (const sz of [-1, 1]) {
    box(hull, m.graphite, [innerW, rimY - floorY, 0.03], [0, (rimY + floorY) * 0.5, sz * (innerD * 0.5 - 0.015)], {
      chamfer: 0.02, fillet: 0.008, bevel: 0.007,
    })
  }
  for (const sx of [-1, 1]) {
    box(hull, m.graphite, [0.03, rimY - floorY, innerD], [sx * (innerW * 0.5 - 0.015), (rimY + floorY) * 0.5, 0], {
      chamfer: 0.02, fillet: 0.008, bevel: 0.007,
    })
  }

  // Moulded suspension pad with four bays cut into it.
  box(hull, m.rubber, [innerW - 0.05, 0.06, innerD - 0.05], [0, floorY + 0.04, 0], {
    chamfer: 0.04, fillet: 0.014, bevel: 0.01,
  })
  for (const sx of [-1, 1]) {
    for (const sz of [-1, 1]) {
      box(hull, m.ink, [innerW * 0.4, 0.05, innerD * 0.38], [
        sx * innerW * 0.23, floorY + 0.055, sz * innerD * 0.22,
      ], { chamfer: 0.045, fillet: 0.016, bevel: 0.01 })
    }
  }
  ...
  for (const sx of [-1, 1]) {
    ...
    const z = innerD * 0.22
    hull.add(cylinder(m.shellShade, 0.088, 0.22, [x, floorY + 0.16, z], AXIS_Y, 14))
    hull.add(cylinder(m.graphiteEdge, 0.094, 0.03, [x, floorY + 0.25, z], AXIS_Y, 14))
    hull.add(cylinder(m.amberPaint, 0.05, 0.028, [x, floorY + 0.275, z], AXIS_Y, 8))
    hull.add(cylinder(m.ink, 0.096, 0.022, [x, floorY + 0.12, z], AXIS_Y, 14))
  }
  box(hull, m.graphiteEdge, [innerW * 0.36, 0.09, innerD * 0.3], [-innerW * 0.23, floorY + 0.11, -innerD * 0.22], {
...
function hullBody(hull: Group, m: CargoMaterials, bundle: CargoMaterialBundle): void {
  ...
  for (const sx of [-1, 1]) {
    for (const sz of [-1, 1]) {
      box(hull, m.graphiteEdge, [0.12, bodyHeight, 0.12], [
        sx * (WIDTH * 0.5 - 0.06), bodyY, sz * (DEPTH * 0.5 - 0.06),
      ], { chamfer: 0.042, fillet: 0.013, bevel: 0.01 })
      box(hull, m.amberPaint, [0.13, 0.055, 0.13], [
        sx * (WIDTH * 0.5 - 0.06), 0.028, sz * (DEPTH * 0.5 - 0.06),
      ], { chamfer: 0.032, fillet: 0.011, bevel: 0.009 })
    }
  }
  ...
  plaque(hull, m, stripe, [0.3, 0.07], [0, SKIRT * 0.5, frontZ], 'front', m.ink)
  boltRun(hull, m.steel, [-0.36, bodyY + 0.22, -frontZ], [0.36, bodyY + 0.22, -frontZ], 5, 0.015, 'back')

  for (const sx of [-1, 1]) {
    recessedHandle(hull, m, [0.26, 0.1], [sx * (WIDTH * 0.5 + 0.004), bodyY, 0], sx > 0 ? 'right' : 'left')
  }
}

function lidBody(lid: Group, m: CargoMaterials): void {
  box(lid, m.shellLight, [WIDTH + 0.02, LID, DEPTH + 0.02], [0, LID * 0.5, DEPTH * 0.5 - 0.04], {
    chamfer: 0.07, fillet: 0.022, bevel: 0.016, capChamfer: 0.045,
  })
  box(lid, m.ink, [WIDTH - 0.26, 0.03, DEPTH - 0.2], [0, LID - 0.008, DEPTH * 0.5 - 0.04], {
    chamfer: 0.05, fillet: 0.016, bevel: 0.01,
  })
  // Underside gasket, which is the face a thrown-back lid actually shows.
  box(lid, m.rubber, [WIDTH - 0.14, 0.02, DEPTH - 0.14], [0, -0.006, DEPTH * 0.5 - 0.04], {
    chamfer: 0.035, fillet: 0.012, bevel: 0.008,
  })
  for (const x of [-0.3, 0.3]) {
    lid.add(prism(m.graphiteEdge, [0.13, 0.09, 0.08], [x, LID * 0.55, 0.02], {
      chamfer: 0.026, fillet: 0.009, bevel: 0.007,
    }))
    lid.add(cylinder(m.steel, 0.022, 0.17, [x, LID * 0.55, 0], AXIS_X, 8))
  }
}
...
function build(): { root: Group; hull: Group; lid: Group; sockets: OpenCrateSockets; bundle: CargoMaterialBundle } {
  ...
  interior(hull, m)
  lid.position.set(0, HEIGHT - LID, -(DEPTH * 0.5 - 0.04))
  lidBody(lid, m)
  lid.rotation.x = -1.92

  const floorY = SKIRT + 0.07
  const sockets: OpenCrateSockets = {
    loot_bay_a: socket('loot_bay_a', [-(WIDTH - WALL * 2) * 0.23, floorY + 0.09, -(DEPTH - WALL * 2) * 0.22]),
    loot_bay_b: socket('loot_bay_b', [(WIDTH - WALL * 2) * 0.23, floorY + 0.09, -(DEPTH - WALL * 2) * 0.22]),
    lid_hinge: socket('lid_hinge', [0, HEIGHT - LID, -(DEPTH * 0.5 - 0.04)]),
