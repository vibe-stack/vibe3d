...
---
asset_id: "asset.architecture.modular-building-connectors.floor-slab-tile"
source_label: "floor slab tile"
...
- **Asset ID:** asset.architecture.modular-building-connectors.floor-slab-tile
- **Type:** architectural connector
- **Source:** prop-list.md:255
...
![floor slab tile reference render](floor-slab-tile.png)

## Intent

The **floor slab tile** is a architectural connector for the Axiom Relay kit. It exists to support
the grid-aligned chassis that lets every district feel related while retaining local dressing. Its primary read should be **floor slab tile** as a useful,
...
- **Blockout envelope:** 2.0 m W × 2.0 m D × 0.20 m H; four-way snap edges on the 1 m grid.
...
Make this the canonical building floor module: a shallow structural tray with a hard perimeter, recessed removable service panel, four corner mounts, and one restrained status strip. It must read as a surface that can tile into a room, not as a decorative platform.

The construction follows the shared **chassis → service → signal** order.
Show the load path, removable service panel, drainage or cable access, and the hard contact face below. Adjacent tiles must meet without visible gaps or accidental z-fighting. Keep the strongest value break on the
...
1. Block the 2 m square tile and its four snap edges.
2. Add the structural tray, corner mounts, and recessed service panel.
3. Add optional material variants for clean civic, industrial, military, and locally repaired floors without changing the connector contract.
4. Validate a 2 × 2 tile patch and a wall-to-floor junction before approving the asset.
