...
---
asset_id: "asset.architecture.modular-building-connectors.building-threshold"
source_label: "building threshold"
...
- **Asset ID:** asset.architecture.modular-building-connectors.building-threshold
- **Type:** architectural connector
- **Source:** prop-list.md:264
...
![building threshold reference render](building-threshold.png)

## Intent

The **building threshold** is a architectural connector for the Axiom Relay kit. It exists to support
the grid-aligned chassis that lets every district feel related while retaining local dressing. Its primary read should be **building threshold** as a useful,
...
- **Blockout envelope:** 2.0 m W × 0.50 m D × 0.20 m H transition strip; aligns with the 1 m floor grid.
...
Create a durable threshold that resolves the floor, door, seal, and approach relationship. It should read as a physically loaded transition, not a decorative strip.

The construction follows the shared **chassis → service → signal** order.
Include a replaceable wear plate, drainage channel, cable crossing option, and a hard clearance boundary for moving doors. Keep the strongest value break on the
...
1. Block the standard door-width transition and floor alignment.
2. Add wear plate, seal, drainage, and optional cable crossing.
3. Validate with interior floors, exterior roads, ramps, gates, and damaged states.
