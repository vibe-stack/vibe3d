...
---
asset_id: "asset.architecture.modular-building-connectors.wall-end-cap"
source_label: "wall end cap"
...
- **Asset ID:** asset.architecture.modular-building-connectors.wall-end-cap
- **Type:** architectural connector
- **Source:** prop-list.md:260
...
![wall end cap reference render](wall-end-cap.png)

## Intent

The **wall end cap** is a architectural connector for the Axiom Relay kit. It exists to support
the grid-aligned chassis that lets every district feel related while retaining local dressing. Its primary read should be **wall end cap** as a useful,
...
- **Blockout envelope:** 0.50 m W × 0.25 m D × 3.0 m H wall termination; accepts one 0.25 m wall run.
...
Make a deliberate wall termination with a protected edge, inset service seam, and optional fascia strip. The end cap must look finished in a street view and serviceable in a close view.

The construction follows the shared **chassis → service → signal** order.
Close the structural and weather layers while providing a small access route for cables or pipes. Keep the strongest value break on the
...
1. Block the single wall termination and its floor/ceiling contacts.
2. Add the edge protection, fascia, and service seam.
3. Test open-ended corridors, facade edges, and damaged variants.
