...
---
asset_id: "asset.architecture.modular-building-connectors.wall-return"
source_label: "wall return"
...
- **Asset ID:** asset.architecture.modular-building-connectors.wall-return
- **Type:** architectural connector
- **Source:** prop-list.md:261
...
![wall return reference render](wall-return.png)

## Intent

The **wall return** is a architectural connector for the Axiom Relay kit. It exists to support
the grid-aligned chassis that lets every district feel related while retaining local dressing. Its primary read should be **wall return** as a useful,
...
- **Blockout envelope:** 1.0 m W × 1.0 m D × 3.0 m H return module; accepts one wall run and one perpendicular short return.
...
Build a short perpendicular return that gives walls depth, cover, and a readable termination. Keep the service recess on the protected face and the exterior edge structurally capped.

The construction follows the shared **chassis → service → signal** order.
Use the return to hide a cable/pipe turn or create a cover pocket without producing a snag-heavy silhouette. Keep the strongest value break on the
...
1. Block the main wall and short perpendicular return.
2. Add the edge cap, service cavity, and lower contact condition.
3. Validate as both a facade reveal and a gameplay cover module.
