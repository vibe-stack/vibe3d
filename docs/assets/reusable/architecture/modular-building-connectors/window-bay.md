...
---
asset_id: "asset.architecture.modular-building-connectors.window-bay"
source_label: "window bay"
...
- **Asset ID:** asset.architecture.modular-building-connectors.window-bay
- **Type:** architectural connector
- **Source:** prop-list.md:263
...
![window bay reference render](window-bay.png)

## Intent

The **window bay** is a architectural connector for the Axiom Relay kit. It exists to support
the grid-aligned chassis that lets every district feel related while retaining local dressing. Its primary read should be **window bay** as a useful,
...
- **Blockout envelope:** 2.0 m W × 0.25 m D × 2.0 m H opening module; accepts standard 1.5 m W × 1.4 m H glazing.
...
Build the complete wall opening with a structural frame, gasketed glazing seat, sill, lintel, and optional exterior shading attachment. It should accept industrial, laboratory, luxury, and storefront inserts.

The construction follows the shared **chassis → service → signal** order.
Show the frame depth, drainage sill, replacement clearance, and the attachment route for shutters or sunshades. Keep the strongest value break on the
...
1. Block the wall-sized opening and the standard glazing seat.
2. Add frame, sill, lintel, gasket, and exterior attachment sockets.
3. Validate small industrial, laboratory, luxury, and storefront window variants.

## Component inventory

- **blocking: window frame** — [window frame](../windows/window-frame.md) — shared frame insert.
...
- **blocking component:** [window frame](../windows/window-frame.md) — shared frame insert.
