...
---
asset_id: "asset.architecture.gate-closure.gate-wall-return"
source_label: "gate wall return"
...
- **Asset ID:** asset.architecture.gate-closure.gate-wall-return
- **Type:** architectural connector
- **Source:** prop-list.md:272
...
![gate wall return reference render](gate-wall-return.png)

## Intent

The **gate wall return** is a architectural connector for the Axiom Relay kit. It exists to support
the grid-aligned chassis that lets every district feel related while retaining local dressing. Its primary read should be **gate wall return** as a useful,
...
- **Blockout envelope:** 2.0 m W × 1.0 m D × 3.0 m H wing-wall module; accepts a gate post and perimeter wall.
...
Create the short wing wall that makes a gate feel embedded in a perimeter rather than floating between two posts. Give it a protected service face and an authored cover edge.

The construction follows the shared **chassis → service → signal** order.
Resolve wall thickness, fence or barrier attachment, foundation anchoring, and the protected cable path into the gate post. Keep the strongest value break on the
...
1. Block the wing wall and its gate-post/perimeter-wall faces.
2. Add foundation anchor, fence attachment, service recess, and cover edge.
3. Validate left/right mirrored placement and damaged wall variants.
