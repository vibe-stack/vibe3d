...
---
asset_id: "asset.architecture.gate-closure.gate-post-pair"
source_label: "gate post pair"
...
- **Asset ID:** asset.architecture.gate-closure.gate-post-pair
- **Type:** architectural connector
- **Source:** prop-list.md:270
...
![gate post pair reference render](gate-post-pair.png)

## Intent

The **gate post pair** is a architectural connector for the Axiom Relay kit. It exists to support
the grid-aligned chassis that lets every district feel related while retaining local dressing. Its primary read should be **gate post pair** as a useful,
...
- **Blockout envelope:** 2.0 m W × 1.0 m D × 4.0 m H paired jamb towers; standard clear gate span 4.0 m.
...
Make the paired gate posts carry the silhouette and load path: thick jamb towers, protected hinge or track seats, service recesses, warning lights, and clean wall attachment faces.

The construction follows the shared **chassis → service → signal** order.
Show the hinge/track load, access panels, power/data entry, and safe vehicle clearance through the opening. Keep the strongest value break on the
...
1. Block the paired jamb towers and standard clear vehicle span.
2. Add hinge or track seats, control recesses, wall attachment faces, and signal lights.
3. Validate checkpoint, perimeter, bunker, and large hero gate variants.
